...
import { testSaga, runs, selects, spawns } from '../../lib/testHelpers';
...
test('Test helper with mocked call to sub saga', async () => {
  await testSaga(watchForUserSelectorToCountIfNotChangedWithing3s)
    ...
    .which(runs(sleep).receiving(), spawns(increaseCounter).receiving()) // TODO: Do we need an assertion for calls that are supposed to be made, but passed through / actually executed? Or maybe shouldn't `callEnv` be mocked (usually) because of the side-effects which can't be replicated here?
