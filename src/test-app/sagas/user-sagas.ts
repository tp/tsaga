...
import { withEnv } from '../../lib/helpers';
...
export const increaseCounter = withEnv(({ dispatch, select, run }: AppEnv, increase: number) => {
  const count = select(getCount);
  console.error(`about to set new count:`, count + increase);
